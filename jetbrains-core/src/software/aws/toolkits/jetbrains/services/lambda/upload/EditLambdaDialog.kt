...
class EditLambdaDialog(
    ...
    private val s3Client: S3Client = AwsClientManager.getInstance(project).getClient()
    private val iamClient: IamClient = AwsClientManager.getInstance(project).getClient()
    ...
    init {
        ...
        view.iamRole.populateValues(selected = role) {
            iamClient.listRolesFilter { it.assumeRolePolicyDocument().contains(LAMBDA_PRINCIPAL) }
                ...
                .sortedWith(Comparator.comparing<IamRole, String>(Function { it.name }, String.CASE_INSENSITIVE_ORDER))
                .toList()
        }
    ...
    override fun doValidate(): ValidationInfo? {
        ...
        updateAction.isEnabled = validateSettings == null // We can update, but not deploy
    ...
    private fun updateLambda() {
        if (updateAction.isEnabled) {
            ...
            val functionDetails = viewToFunctionDetails()
            val lambdaClient: LambdaClient = AwsClientManager.getInstance(project).getClient()
